...
function rewrite(test, url) {
  if(test.option && test.option.loader &&
    test.option.loader.rewrite && url in test.option.loader.rewrite) {
    return test.option.loader.rewrite[url];
  }
  const manifest = test.manifest;
  if(manifest.option && manifest.option.loader &&
    manifest.option.loader.rewrite && url in manifest.option.loader.rewrite) {
